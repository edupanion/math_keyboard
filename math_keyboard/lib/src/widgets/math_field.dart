    ...
    for (final config in configs) {
      ...
      if (config.keyboardCharacters.any((element) => element.toLowerCase() == lowerCaseCharacter)) {
        ...
        if (basicConfig.args != null) {
          _controller.addFunction(basicConfig.value, basicConfig.args!, basicConfig.suffixArgs);
        } else {
...
class MathFieldEditingController extends ChangeNotifier {
  ...
  void addFunction(String tex, List<TeXArg> args, [List<TeXArg>? suffixArgs]) {
    currentNode.removeCursor();
    final func = TeXFunction(tex, currentNode, args, null, suffixArgs);
